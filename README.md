...
| Platform | File                  | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| macOS    | `Tyvox-*.dmg`         | Unsigned (ARM and Intel builds). macOS may block the app or report it as malware on first launch — this is Gatekeeper blocking an unsigned app, not an actual threat. Fix: try to open it once, then go to System Settings → Privacy & Security and click **Open Anyway** next to Tyvox. If the dialog says the app is damaged (common on Apple Silicon), run `sudo xattr -rd com.apple.quarantine /Applications/Tyvox.app && codesign --force --deep --sign - /Applications/Tyvox.app`, then open it. If macOS moved the app to the Trash, reinstall it from the dmg first. |
...
The app asks for two permissions — **microphone** and **accessibility** (global hotkey + simulated paste). The settings page walks you through both on first launch. On macOS, each update invalidates the accessibility grant (unsigned builds change identity); the app clears the stale entry itself — you only need to re-toggle Tyvox in System Settings.

### Auto-update

- **Windows / Linux AppImage** — updates download in the background and apply on restart.
- **macOS** (unsigned — the platform blocks auto-update) and **deb** — the About tab flags new versions and links to the release page.
